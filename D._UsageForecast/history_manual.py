...
#%%px
#Select data
selection = ((data['Now'] - data['Creation-week']) > 34)&((data['Now'] - data['FirstUsage']) > 34)&((data[70] - data[1]) != 0)
...
subplot(224)
t = df_ts_states.irow([row])
rt = pd.rolling_sum(t, window=26, axis=1).values[0,25:]
plt.plot(rt)
subplot(223)
t = df_time_series.irow([row])
rt = pd.rolling_sum(t, window=26, axis=1).values[0,25:]
plt.plot(rt)
...
for row in range(0, df_ts_states.shape[0]):
    train = df_ts_states.irow([row]).values[0][50:70]
...
#session.commit("Advanced Markov Chains. Supervised learning. y_scores for all features added.")

# <codecell>

session.commit("Advanced Markov Chains. Report 1.")