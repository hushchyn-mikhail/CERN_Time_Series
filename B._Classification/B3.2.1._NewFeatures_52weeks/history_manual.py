...
#Select data
selection = ((data['Now'] - data['Creation-week']) > 52)&((data['Now'] - data['FirstUsage']) > 52)&((data[53] - data[1]) != 0)
...
y_true = ((data_sel[104] - data_sel[53]) == 0).values*1

# <codecell>

for i in df.columns:
    plt.subplot(1,1,1)
    plt.hist(df[i][y_true == 1].values, label='1', color='b', alpha=0.5, bins = 20)
    plt.hist(df[i][y_true == 0].values, label='0', color='r', alpha=0.5, bins = 20)
    plt.legend(loc = 'best')
    plt.title(i)
    plt.show()
