...
df['ProcessingPass'] = data_sel['ProcessingPass'].values

stripping = []
for i in range(0, len(df.ProcessingPass.values)):
    substring = pattern1.findall(df.ProcessingPass.values[i])
    if substring == []:
        stripping.append('None')
    else:
        stripping.append(substring[0])
txt_stripping = stripping
df['Stripping'] = stripping

#Unique stripping values
strippings = np.unique(df.Stripping.values)
print len(strippings)
print strippings

#Histogramm of the processing passes distribution
mapping = dict(zip(strippings, range(0, len(strippings))))
st_indexes = df.replace({'Stripping':mapping}).Stripping.values
df['Stripping'] = st_indexes + 100*data_sel['Type'].values

print np.unique(df['Stripping'].values)

plt.hist(df['Stripping'].values, bins=len(np.unique(df['Stripping'].values)))
...
i = 0
for a in np.unique(df['Stripping'].values):
    print df[df['Stripping']==a].shape, a
    i=i+1
print i
...
#Union
df_stripping = df['Stripping'].copy()
for i in range(0,df_stripping.shape[0]):
    if df_stripping[i]>=100:
        if (df_stripping[i]!=100)&(df_stripping[i]!=107)&(df_stripping[i]!=110):
            df_stripping[i]=101
    if df_stripping[i]<100:
        if (df_stripping[i]==8):
            df_stripping[i]=9

df['Stripping'] = df_stripping.values
np.unique(df_stripping.values)
...
#Check >0
for i in np.unique(df['Stripping'].values):
    print 'Group Number is ', i, ' : ', len(y_true[df['Stripping'].values==i]) - y_true[df['Stripping'].values==i].sum()
...
groups = np.unique(df['Stripping'].values)
...
for i in range(0, len(groups)):
    signal_data[i] = data_storage.DataStorageDF(df[(y_true == 1)&(df['Stripping'] == groups[i])])
    bck_data[i] = data_storage.DataStorageDF(df[(y_true == 0)&(df['Stripping'] == groups[i])])
...
for i in range(0, len(groups)):
    y_true_wh = np.concatenate((np.ones((report_one.prediction_sig['classifier'][(one_signal_test.get_data(['Stripping']).values[:,0]==groups[i])]).shape[0]),\
                                np.zeros((report_one.prediction_bck['classifier'][(one_bck_test.get_data(['Stripping']).values[:,0]==groups[i])]).shape[0])),\
                               axis=0)
    y_score_wh = np.concatenate((report_one.prediction_sig['classifier'][(one_signal_test.get_data(['Stripping']).values[:,0]==groups[i])],\
                                report_one.prediction_bck['classifier'][(one_bck_test.get_data(['Stripping']).values[:,0]==groups[i])]),\
                               axis=0)
